...
else echo "Working Run_2 folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2; date
fi

# Change directory
cd $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2

if [ -d "Partition_${SLURM_ARRAY_TASK_ID}" ]
then echo "Working Partition_${SLURM_ARRAY_TASK_ID} folder exist"; echo "Let's move on."; date
else echo "Working Partition_${SLURM_ARRAY_TASK_ID} folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}; date
fi
...
# Change directory 
cd $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}

# Cat file of protein headers and start while loop
cat $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/protein.names.${SLURM_ARRAY_TASK_ID}.txt | \
while read protein 
do 
echo ${protein}
...
cat $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}/blastp_vs_uniref90.outfmt6_protein.* \
> $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/blastp_vs_uniref90.outfmt6_partition${SLURM_ARRAY_TASK_ID}.txt

#--------------------------------------------------------------------------------

# Housekeeping - remove intermediate files
rm $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}/protein.*.partition.${SLURM_ARRAY_TASK_ID}.fa
rm $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}/blastp_vs_uniref90.outfmt6_protein.*
rm $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/protein.names.${SLURM_ARRAY_TASK_ID}.txt
rm -R $WORKING_FOLDER/Gene_ontology/blastp_array/Run_2/Partition_${SLURM_ARRAY_TASK_ID}
